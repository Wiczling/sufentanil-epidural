...
data<-pkmod3ss %>% 
  ...
  summarize(absq0.5 =  quantile(ABS,probs=c(0.5)),
            ...
            contq0.95  =quantile(CENT,probs=c(0.95))) %>%
  mutate(tinf="ss")%>%
  mutate(rate = 3+10.9/1.99*3*exp(-0.162*time))

p0 <- data %>%
  ggplot(aes(x=time, y = rate, color=as.factor(tinf)))+
  geom_line()+
  labs(title = "Sufentanil infusion rate (Rss=3mcg/h)", x = element_blank(), y = "Rate, mcg/h", color="Tinf")+
...
p1 <- data %>%
  ...
  scale_y_continuous(trans='log10',
                      breaks=trans_breaks('log10', function(x) 10^x),
                      labels=trans_format('log10', math_format(10^.x)))+
  coord_cartesian(ylim=c(0.01,20))+ theme(legend.position = "none")
...
p2 <- data %>%
  ...
  scale_y_continuous(trans='log10',
                      breaks=trans_breaks('log10', function(x) 10^x),
                      labels=trans_format('log10', math_format(10^.x)))+
  coord_cartesian(ylim=c(0.01,200))+ theme(legend.position = "none")

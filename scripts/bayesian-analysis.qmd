...
K10 = as.double(ptable$mean[6])
K13 = as.double(ptable$mean[7])
K31 = as.double(ptable$mean[8])
beta_abs= 0.5*(K13+K31+K10-sqrt((K13+K31+K10)^2-4*K31*K10))
alpha_abs = K31*K10/beta_abs

K10 = as.double(ptable$mean[2])/as.double(ptable$mean[4])
K13 = as.double(ptable$mean[3])/as.double(ptable$mean[4])
K31 = as.double(ptable$mean[3])/as.double(ptable$mean[5])
beta_dis= 0.5*(K13+K31+K10-sqrt((K13+K31+K10)^2-4*K31*K10))
alpha_dis = K31*K10/beta_dis
...
## shrinkage ?
```{r}
errors <-  posterior::as_draws_df(mod_to_plot$draws("etaM"))%>%
  mutate(across(starts_with("eta"), ~log(.x)))

bbr.bayes::shrinkage(errors=errors,"etaM",group_idx = 1, use_sd = TRUE)
```
...
# M4: Different Priors (Based on https://pubmed.ncbi.nlm.nih.gov/8533912/)
```{r  model4, eval = FALSE}
...
## LOO-based
```{r compare-models-log-lik, eval=FALSE}
  # mod5  <- read_model(file.path(model_dir, "5"))
...
mod2_gq  <- read_model(file.path(model_dir, "2_gq"))
mod2_gq <- submit_model(mod2_gq,.overwrite = TRUE)
...
files <- list.files(path = here::here(mod2_gq$absolute_model_path,"2_gq-output"),
                    pattern = "\\.csv$", 
                    ignore.case = TRUE, full.names=TRUE)

temp <- cmdstanr::read_cmdstan_csv(files)
ll2 <- as_draws_array(temp$generated_quantities)
...
ggsave(filename = here::here(manuscript_figures_dir, "figure1.png"),plot = figure_1, width=12, height=10, dpi=600, units="cm")
ggsave(filename = here::here(manuscript_figures_dir, "figure2.png"),plot = figure_2, width=12, height=10, dpi=600, units="cm")
ggsave(filename = here::here(manuscript_figures_dir, "figure3.png"),plot = figure_3, width=12, height=10, dpi=600, units="cm")
ggsave(filename = here::here(manuscript_figures_dir, "figure4.png"),plot = figure_4, width=12, height=10, dpi=600, units="cm")
ggsave(filename = here::here(manuscript_figures_dir, "figure5.png"),plot = figure_5, width=12, height=10, dpi=600, units="cm")

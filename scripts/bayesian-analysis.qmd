...
# mod1  <- read_model(file.path(model_dir, "1"))
# mod1_gq <- copy_model_as_stan_gq(mod1, .overwrite = TRUE)
# mod3  <- read_model(file.path(model_dir, "3"))
# mod3_gq <- copy_model_as_stan_gq(mod3, .overwrite = TRUE)
# mod5  <- read_model(file.path(model_dir, "5"))
...
mod1_gq <- read_fit_model(read_model(here::here(model_dir, "1_gq")))
mod3_gq <- read_fit_model(read_model(here::here(model_dir, "3_gq")))
mod5_gq <- read_fit_model(read_model(here::here(model_dir, "5_gq")))

ll1<-mod1_gq$draws()
ll3<-mod3_gq$draws()
ll5<-mod5_gq$draws()

loo1 <- loo(subset_draws(ll1,  variable = "log_lik"), 
            pars="log_lik", cores== getOption("mc.cores", 4)) 
loo3 <- loo(subset_draws(ll3,  variable = "log_lik"), 
            pars="log_lik$", cores== getOption("mc.cores", 4))  
loo5 <- loo(subset_draws(ll5,  variable = "log_lik"),
            pars="log_lik$", cores== getOption("mc.cores", 4))

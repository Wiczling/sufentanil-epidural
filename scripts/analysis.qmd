...
mod1 <- mod1 %>% 
  add_tags("2 cmp disposition model F=1, ABS: 1-st order")%>% 
  add_tags("Etas fixed to zero except CL")
mod1 <- mod1 %>% add_notes(" ")
```
...
mod2 <- mod2 %>% 
  add_tags("2 cmp disposition model (fixed), F estimated, ABS: simple")%>% 
  add_tags("Eta for ka estimated, fixed for other to 0.1")
mod2 <- mod2 %>% add_notes(" ")
```
```{r model3, eval = FALSE}
...
mod3 <- mod3 %>% 
  add_tags("2 cmp disposition model (fixed), F=1, ABS: 2 cmp model")%>% 
  add_tags("Eta for k41 estimated, fixed for other to 0.1/0")
mod3 <- mod3 %>% add_notes(" ")
```
...
```{r ofv-summary}
summary_log(model_dir, .include = 1:3) %>%
